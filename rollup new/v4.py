...
5. 不要输出 JSON。
6. 不要输出日期范围和源日报链接。
7. 不要输出个人姓名、个人 mention 或普通 @姓名。
8. 如输入中存在关键数量或明确枚举成果，优先保留数量。
9. 不要编造输入中不存在的事实。
...
def generate_rollup_card_content(target_dept, long_markdown, week_info):
    ...
    try:
        ...
        return normalize_rollup_markdown(summary_prefix + strip_markdown_wrapper(llm_result))
    except Exception as e:
        print(f"⚠️ [Card][{dept_name}] AI 生成失败，使用 fallback: {e}")
        return normalize_rollup_markdown(summary_prefix + fallback_format_content(card_input_markdown, max_len=20000))
...
def run_rollup_for_target_dept(target_dept, dept_by_id, children_map, week_info):
    ...
    try:
        ...
        final_body = normalize_rollup_markdown(final_body)

        header = build_rollup_header(target_dept, leaf_depts, week_info)
        final_markdown = header + "\n\n" + final_body.strip()
        final_markdown = normalize_rollup_markdown(final_markdown)
