# -*- coding: utf-8 -*-
"""
非原子池 PM 日报 -> 周报生成脚本（固定 weekly）

设计目标：
1. 配置、模型调用、周报生成、校验、写回、消息推送尽量对齐“基于原子池的周报脚本”。
2. 不再读取 atomic_pool JSON，而是读取原始日报 docJson。
3. 从原始日报中定位 PM 日报板块，按 mention 人头标题拆分 person_blocks。
4. 默认使用规则将 person_blocks 全量转换为临时 weekly items，避免前置 LLM 抽取漏项。
5. 后续复用原子池周报的 timeline -> trend -> final -> coverage -> repair 链路。
...
def build_weekly_note_title(week_info, project_name):
    ...
    return f"{year}#W{week_info['week_number']:02d} {project_name}周报"
...
def get_or_create_weekly_note(project, week_info):
    ...
    note_guid, _ = create_workspace_note(
        ...
        title=title,
        tags=get_weekly_config(project, "weekly_note_tags", ["周报", "AI", "非原子池"])
    )
...
def send_weekly_messages(note_url, note_title, project, final_markdown):
    try:
