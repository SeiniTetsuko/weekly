...
def _sanitize_llm_json_text(text):
    """
    修复 LLM 输出 JSON 中常见问题：
    - markdown code fence
    - 零宽字符 / BOM
    - 非法反斜杠，例如 LGCAA/DRGPA -> LGCAA//DRGPA
    """
    text = strip_markdown_wrapper(text or "")
...
def safe_json_loads(text):
    clean_text = _sanitize_llm_json_text(text)
    ...
    if object_match:
        candidate = _sanitize_llm_json_text(object_match.group(1))
        try:
            return json.loads(candidate, strict=False)
        except Exception as e:
            print(f"⚠️ JSON 对象解析失败: {e}")
            print(candidate[:1000])
            raise

    array_match = re.search(r"(\[.*\])", clean_text, flags=re.DOTALL)
    if array_match:
        candidate = _sanitize_llm_json_text(array_match.group(1))
        try:
            return json.loads(candidate, strict=False)
        except Exception as e:
            print(f"⚠️ JSON 数组解析失败: {e}")
            print(candidate[:1000])
            raise

    print("⚠️ 无法从模型输出中提取 JSON，原始输出前1000字：")
...
def is_important_text(text):
    """
    识别日报中的重要标记。
    支持：[重要]、【重要】、[重点]、【重点】、重要：、重点：
    """
    text = text or ""
    patterns = ["[重要]", "【重要】", "[重点]", "【重点】", "重要：", "重点："]
    return any(p in text for p in patterns)


def clean_important_marker(text):
    """
    清理输出中的重要标记本身，但保留其语义用于 important 标志。
    """
    text = text or ""
    for p in ["[重要]", "【重要】", "[重点]", "【重点】"]:
        text = text.replace(p, "")
    text = re.sub(r"^\s*(重要|重点)\s*[:：]\s*", "", text)
    return text.strip()


def item_has_important_marker(item):
    for block in item.get("content", []) or []:
        if is_important_text(block.get("text", "")):
            return True
    return False



def flatten_content_text(item):
    texts = []
    for block in item.get("content", []) or []:
        text = block.get("text", "")
        if text:
...
def is_completion_candidate_text(text):
    """
    判断一条原子事项是否可能属于“已完成/已上线/已验证/已形成产物”的成果候选。
    这个字段只作为给模型的提示，不直接决定最终输出。
    """
    text = text or ""

    positive_keywords = [
        "完成", "已完成", "完成度", "100%",
        "上线", "已上线", "部署", "已部署", "发布", "交付", "推送",
        "验证通过", "测试通过", "结果一致", "in spec", "correlation良好",
        "准确率", "R^2", "R²", "优于", "提升",
        "实现", "设计实现", "调通", "改造", "集成", "自动化",
        "接口修改", "模型转换", "环境部署", "训练", "测试优化",
        "输出文档", "形成模板", "方案模板", "SOP已准备", "SOP 已准备",
        "输出到", "上传至", "bank建立", "bank 建立", "数据入库",
        "解决", "已解决", "修复", "优化了", "优化", "修改",
        "回溯", "收集", "建立", "打通", "上线前测试"
    ]
    ...
    return any(k in text for k in positive_keywords + important_keywords)


def item_has_completion_candidate(item):
    return is_completion_candidate_text(flatten_content_text(item))
...
def atomic_item_to_rollup_entry(item):
    ...
    return {
        ...
        "important": item_has_important_marker(item),
        "completion_candidate": item_has_completion_candidate(item),
            ...
            for entry in entries or []:
                ...
                for item in entry.get("items", []) or []:
                    ...
                    day_items.append({
                        ...
                        "completion_candidate": bool(item.get("completion_candidate", False)),
...
def get_default_rollup_card_prompt():
    return """你是父部门逐层上报周报的飞书卡片摘要助手。

你会收到完整的父部门逐层上报周报正文。你的任务是生成适合飞书卡片展示的极简摘要。

# 最高优先级原则
1. 只基于输入内容总结，不允许新增事实。
2. 不输出日期范围、周数、源链接。
3. 不输出个人姓名、个人 mention、普通 @姓名。
4. 不输出 Markdown 标题语法，例如 #、##、###。
5. 不输出 emoji。
6. 不输出表格。
7. 不输出 JSON。
8. 不输出代码块。
...
10. 不使用复杂嵌套加粗。
11. 不要在中文字符之间插入空格。
12. 不要在英文技术词、平台名、算法名、指标名、产品名内部插入空格。
13. 不要把 Grafana 写成 G raf ana。
14. 不要把 wafer 写成 wa fer。
15. 不要把 Precision 写成 Prec ision。
16. 不要把 Slope 写成 Sl ope。
17. 不要把 Defocus 写成 Def ocus。
18. 不要把 EQPDATA 写成 EQ PD ATA。
19. 不要把 DDCPB 写成 DDCP B。
20. 不要把 AutoML 写成 auto ml。

# 内容选择规则
请综合全文，重点保留：
1. 本周已完成、已上线、已部署、已交付的成果。
2. 已验证通过、测试通过、结果一致、in spec、完成度明确的事项。
3. 明确数量、完成度、枚举对象。
4. 标记为 [重要]、【重要】、[重点]、【重点】 的事项。
5. 明确阻塞或明确所需帮助。
6. general 的下一步计划。

不要展开普通进行中、讨论中、调研中、计划中事项。
...
**困难/帮助**
• 如果有明确阻塞或所需帮助，写 1~2 条
• 如果没有，写：暂无明确阻塞性问题或外部协助事项

**下一步**
• 如果有明确 general 下一步计划，写 1~2 条
• 如果没有，写：暂无明确下一步计划

# 长度要求
总长度控制在 250~400 字。
每条 bullet 控制在 40~80 字。
如果内容过多，只保留最高优先级事项。
...
1. 必须输出合法 JSON。
2. 不要输出 Markdown 代码块。
3. 所有 summary 必须是单行字符串，不要在字符串内部换行。
4. 如果文本中包含反斜杠 `\\`，必须替换为 `/`，例如 `LGCAA/DRGPA/DRDMP`，不要输出 `LGCAA\\DRGPA\\DRDMP`。
5. 不要输出未转义的双引号。
6. 不要输出注释。
7. 不要在 JSON 后追加解释文字。
  ...
  "issues_support": [
    {
      "leaf_dept_name": "",
      "summary": "",
      "severity": "blocking / support_needed",
      "evidence_dates": ["YYYY-MM-DD"],
      "source_item_ids": [""]
    }
  ],
...
2. core_progress：选出本 batch 最关键的 2~6 条成果。
3. main_progress：按 leaf_dept_name 聚合。每个 leaf_dept_name 在该 platform 下输出 1 条 80~120 字以内的成果总结。
4. involved_departments 必须来自输入中的 leaf_dept_name。
5. important：如果该总结包含或来源于 [重要] / 【重要】 / [重点] / 【重点】 标记事项，设为 true。
6. important_evidence：如果 important=true，列出触发重要标记的原文关键短语；否则输出 []。
7. 不允许输出个人姓名、个人 mention、普通 @姓名。

# completion_candidate 规则
输入 item 中可能包含 completion_candidate 字段。
1. completion_candidate=true 表示代码规则判断该 item 可能包含已完成、已上线、已验证、已形成产物或明确成果。
2. main_progress 应优先从 completion_candidate=true 或 important=true 的 item 中抽取。
3. completion_candidate=false 的内容只有在明确包含重要成果时才纳入。
4. 不要因为措辞不是“完成”就忽略 completion_candidate=true 的内容。

# 成果筛选规则
以下内容应视为“已取得成果/可上报进展”，优先纳入 main_progress：

注意：不要求所有事项都 100% 完成。只要已经产生阶段性结果、指标变化、验证结论、完成度、产物输出、问题定位或可复用能力，也应视为可上报进展。
...
例外：
如果事项包含 [重要] / 【重要】 / [重点] / 【重点】，即使仍在进行中，也必须保留。

# 困难及所需帮助筛选规则
issues_support 只输出非常明确的阻塞或协助事项，必须满足至少一个条件：
1. 明确存在阻塞、无法推进、依赖未满足、接口/权限/数据/资源未提供。
2. 明确可能导致 delay / 延期 / 影响交付 / 影响上线 / 影响验证。
3. 明确需要外部团队、上级、其他平台或某类资源支持。
4. 只是一般问题、讨论、风险猜测、待观察，不要输出。
5. 不要自行推断影响程度。

# 下一步计划筛选规则
next_plan 只输出 general、汇总型下一步计划：
1. 只保留明确的后续推进方向、后续验证、后续上线、后续优化、后续联调。
2. 不输出过细的个人任务。
3. 不输出普通“继续推进”“持续跟进”这类空泛表达。
4. 如果只是由困难直接推导出的动作，但输入没有明确写，不要输出。
5. 下一步计划篇幅要短，每个 leaf_dept_name 在一个 platform 下最多 1 条 general summary。

# 量化要求
1. 1~5 个对象必须列出名称。
2. 6 个及以上必须写明数量，并尽量列出代表项。
3. 不得把明确枚举改写成“多个”“若干”“相关”。
4. 已有数字、完成度、百分比必须保留。
...
你会收到结构化周报草稿，内容来自当前父部门下所有叶子部门的 atomic facts，并且已经删除个人 mention。

你的任务是生成父部门管理层周报正文。

# 输出定位
这是父部门/L4/L3 部门的逐层上报周报，不是完整项目周报。
...
但如果某事项带有 [重要] 或 【重要】，即使不是完成态，也必须体现。
...
4. 严禁输出普通 @姓名。
5. 不输出个人姓名，除非姓名是系统名、项目名或业务名的一部分。
6. 不输出子部门周报链接。
7. 必须保留 platform。
8. 必须保留子部门名称。
9. 必须保留明确数量、完成度、枚举对象。
10. 如果输入中有 [重要] 或 【重要】 标记，必须在输出中体现。
11. 不要输出日期范围、周数、汇总范围等 header，这些由系统自动补充。
12. 不要输出 source_item_ids、evidence_dates、source_url 等中间态字段。
13. 不要输出代码块，不要输出 JSON。

# Markdown 格式要求
1. 章节标题必须严格使用以下四个标题，不要增加 emoji：
   ### 本周关键进展
   ### 本周主要进展
   ### 困难及所需帮助
   ### 下一步计划
2. 标题必须独占一行。
3. 标题后必须空一行再写正文。
4. 不要输出“### 本周关键进展本周...”这种标题和正文粘连格式。
5. 不要在中文字符之间插入空格。
6. 不要在英文技术词、平台名、算法名、指标名、产品名内部插入空格。
7. 不要输出复杂嵌套 Markdown。
8. 不要使用表格。
9. 列表只使用普通 markdown 列表。

# 英文技术词格式要求
不要拆分英文技术词、产品名、平台名、算法名、指标名。
例如：
- Grafana 不要写成 G raf ana
- wafer 不要写成 wa fer
- Defocus 不要写成 Def ocus
- Precision 不要写成 Prec ision
- Slope 不要写成 Sl ope
- Ginko 不要写成 G inko
- EQPDATA 不要写成 EQ PD ATA
- DDCPB 不要写成 DDCP B
- AutoML 不要写成 auto ml
...
用 80~120 字概括当前父部门下 L4/L3 子部门的本周关键成果。

要求：
- 只总结已完成、已上线、已交付、已验证通过、已形成产物的事项。
- 可以提及 [重要] / 【重要】 标记事项。
- 不要写空泛表述，例如“整体进展顺利”“持续推进”。
- 不要输出日期范围、周数、源链接。
...
- **{Platform}**
    - **{Sub Dept}**：80~120 字总结该子部门在该 platform 下本周已完成/已上线/已取得成果的关键进展。

要求：
1. 不要改写 Platform 名称。
2. 不要改写 Sub Dept 名称。
3. 每个子部门总结控制在 80~120 字左右。
4. 可将同一子部门下的多个 bullet 合并为一段，但不能丢失关键数量、完成度、重要事项。
5. 必须保留关键数量、完成度、枚举对象。

### 困难及所需帮助

只输出明确阻塞或明确需要协助的事项。篇幅要短。

输出格式：

- **{Platform}**
    - **{Sub Dept}**：一句话说明明确阻塞或所需帮助。
...
要求：
1. 必须是输入中明确提到的阻塞、依赖、接口、权限、数据、资源或跨团队支持需求。
2. 必须是可能影响交付、上线、验证或进度的事项。
3. 不要输出一般性风险猜测、普通问题或待观察事项。
4. 不要自行推断影响程度。
5. 每个 platform 下每个子部门最多保留 1 条最关键困难/帮助总结。

### 下一步计划

输出 general 的下一步计划，篇幅要短。

输出格式：

- **{Platform}**
    - **{Sub Dept}**：一句话概括后续推进方向。
...
要求：
1. 只保留明确后续推进方向、验证、上线、联调、优化。
2. 不输出过细个人任务。
3. 不输出空泛“继续推进”“持续跟进”。
4. 不从困难中自行推导计划，必须是输入明确存在的计划。
5. 每个 platform 下每个子部门最多保留 1 条 general 下一步计划。

# 内容筛选规则
主要进展优先保留：
1. 含“完成”“已完成”“上线”“已上线”“部署”“已部署”“交付”“发布”的事项。
2. 含“验证通过”“测试通过”“结果一致”“in spec”“完成度100%”的事项。
3. 含明确数量、完成度、枚举对象的事项。
4. 含“输出文档”“形成模板”“完成方案 v1.0”的事项。
5. 含 [重要] 或 【重要】 的事项。
6. 已产生阶段性结果、指标变化、验证结论、完成度、产物输出、问题定位或可复用能力的事项。

弱化或省略：
1. “正在进行”“进行中”“计划”“准备”“预计”“后续考虑”。
2. “讨论”“调研”“确认方向”“尝试”且没有明确结果的事项。
3. 单纯会议、同步、早会观察，除非标记为重要或有明确产出。

# 量化表达要求
1. 输入中明确列出的算法、case、模块、接口、实验、文档、任务项等必须尽量保留。
2. 1~5 个对象：必须列出名称。
3. 6 个及以上：必须写明数量，并尽量列出关键项或代表项。
4. 不得将“完成 A、B、C、D、E 算法”压缩成“完成多个算法”。
5. 不得将“完成 12 个 case”压缩成“完成多个 case”。
6. 如果输入中已有数字，最终输出必须保留该数字。

# 风格要求
1. 客观、中性、专业、简洁。
2. 本周主要进展中每个子部门总结控制在 80~120 字。
3. 困难和下一步计划要更短，不要展开过多。
...
请在不大规模重写的前提下，补回遗漏内容并修复不符合要求的内容。

# 要求
1. 只补充结构化草稿中明确存在的事实。
2. 严禁输出个人 mention。
3. 严禁输出普通 @姓名。
4. 严禁输出个人姓名，除非姓名是系统名、项目名或固定业务名的一部分。
5. 不输出日期范围、周数、汇总范围、源链接等 header。
6. 保留四个章节：本周关键进展、本周主要进展、困难及所需帮助、下一步计划。
7. 困难及所需帮助只补明确阻塞或明确协助事项。
8. 下一步计划只补明确且 general 的后续方向。
9. 保留 platform、leaf_dept_name、数量、完成度、枚举对象。
10. 如果校验结果指出 [重要] / 【重要】 标记内容遗漏，必须补回。
11. 只输出修复后的 Markdown 正文。
...
def fallback_rollup_analyze_batch(batch_state):
    ...
    兜底原则：
    1. progress section 全部进入 main_progress，避免父部门周报全空；
    2. important=true 的任意 section 也进入 main_progress；
    3. issues_support / next_plan 仍按 section 输出；
    4. 兜底不做过度筛选，宁可保留可读内容，也不要输出“暂无”。
    """
    platform = batch_state.get("platform", "部门工作")
    result = {"platform": platform, "core_progress": [], "main_progress": [], "issues_support": [], "next_plan": []}

    leaf_progress_map = OrderedDict()
    leaf_issue_map = OrderedDict()
    leaf_next_map = OrderedDict()
    core_candidates = []
        ...
        for section_name, entries in (project.get("sections", {}) or {}).items():
            for entry in entries or []:
                ...
                for item in entry.get("items", []) or []:
                    ...
                    item_text = f"{project_name}: {tree_md[:500]}"
                    is_important = bool(item.get("important", False))
                    is_completion = bool(item.get("completion_candidate", False))

                    target_map = None

                    # ✅ 兜底放宽：progress 全部保留
                    if section_name == "progress":
                        target_map = leaf_progress_map

                    # ✅ 重要标记无论在哪个 section 都进入主要进展，避免遗漏
                    elif is_important:
                        target_map = leaf_progress_map

                    elif section_name == "issues_support":
                        target_map = leaf_issue_map

                    elif section_name == "next_plan":
                        target_map = leaf_next_map

                    if target_map is None:
                        continue

                    target_map.setdefault(leaf_dept_name, {
                        "texts": [],
                        "dates": set(),
                        "ids": [],
                        "important": False,
                    ...
                    target_map[leaf_dept_name]["texts"].append(item_text)

                    if entry.get("date"):
                        target_map[leaf_dept_name]["dates"].add(entry["date"])

                    if item.get("item_id"):
                        target_map[leaf_dept_name]["ids"].append(item.get("item_id"))

                    if is_important:
                        target_map[leaf_dept_name]["important"] = True
                        target_map[leaf_dept_name]["important_evidence"].append(item_text[:120])

                    # completion_candidate 或 important 进入核心候选
                    if is_completion or is_important:
                        core_candidates.append({
                            "summary": f"{leaf_dept_name}：{item_text[:300]}",
                            "involved_departments": [leaf_dept_name],
                            "important": is_important,
                            "important_evidence": [item_text[:120]] if is_important else [],
                            "evidence_dates": [entry.get("date")] if entry.get("date") else [],
                            "source_item_ids": [item.get("item_id", "")]
                        })

    def map_to_items(src_map, max_len=500):
        out = []
        for leaf_dept_name, data in src_map.items():
            if not data["texts"]:
                continue

            summary = "；".join(data["texts"][:8])
            if len(summary) > max_len:
                summary = summary[:max_len] + "..."

            out.append({
                "leaf_dept_name": leaf_dept_name,
                "summary": summary,
                "important": data.get("important", False),
                "important_evidence": data.get("important_evidence", [])[:3],
                "evidence_dates": sorted(data.get("dates", [])),
                "source_item_ids": [x for x in data.get("ids", []) if x]
            })

        return out

    result["main_progress"] = map_to_items(leaf_progress_map, max_len=700)
    result["issues_support"] = map_to_items(leaf_issue_map, max_len=300)
    result["next_plan"] = map_to_items(leaf_next_map, max_len=300)

    # 核心进展优先来自 completion_candidate/important；若没有，则取主要进展前 3 条
    result["core_progress"] = core_candidates[:3] or [
        {
            "summary": item["summary"],
            "involved_departments": [item["leaf_dept_name"]],
            "important": item.get("important", False),
            "important_evidence": item.get("important_evidence", []),
            "evidence_dates": item.get("evidence_dates", []),
            "source_item_ids": item.get("source_item_ids", [])
        }
        for item in result["main_progress"][:3]
    ]
...
def normalize_rollup_item(item):
    return {
        "leaf_dept_name": (item.get("leaf_dept_name") or "").strip(),
        "summary": (item.get("summary") or "").strip(),
        "involved_departments": item.get("involved_departments", []) or [],
        "important": bool(item.get("important", False)),
        "important_evidence": item.get("important_evidence", []) or [],
...
def dedupe_rollup_items(items):
    ...
    for item in items or []:
        key = (
            item.get("leaf_dept_name"),
            tuple(item.get("involved_departments", [])),
            item.get("summary"),
...
def render_rollup_platform_section(platform_map, section_key, empty_text="本周暂无明确内容。"):
    ...
    for platform, sections in platform_map.items():
        ...
        if section_key == "core_progress":
            for item in items:
                involved = item.get("involved_departments") or []
                dept_text = f"（涉及部门：{'、'.join(involved)}）" if involved else ""
                important_tag = "【重要】" if item.get("important") else ""
                parts.append(f"    - {important_tag}{item.get('summary', '')}{dept_text}")
        else:
            grouped_by_dept = OrderedDict()
            for item in items:
                dept_name = item.get("leaf_dept_name") or "未知部门"
                grouped_by_dept.setdefault(dept_name, [])
                grouped_by_dept[dept_name].append(item)

            for dept_name, dept_items in grouped_by_dept.items():
                summaries = []
                important = False
                ...
                dept_summary = "；".join(summaries)
                if len(dept_summary) > 260:
                    dept_summary = dept_summary[:260] + "..."
                important_tag = "【重要】" if important else ""
                parts.append(f"    - **{dept_name}**：{important_tag}{dept_summary}")
...
def has_rollup_content(platform_map):
    """
    判断 trend 合并结果是否有实际内容。
    如果完全为空，后续会输出“暂无”，因此需要触发兜底重建。
    """
    for _, sections in (platform_map or {}).items():
        for key in ("core_progress", "main_progress", "issues_support", "next_plan"):
            if sections.get(key):
                return True
    return False


def rebuild_rollup_results_with_fallback(rollup_timeline_state, target_dept):
    """
    当模型趋势分析结果为空时，直接用 fallback 对所有 batch 重建，避免写出全“暂无”的周报。
    """
    max_projects = int(get_rollup_config(target_dept, "rollup_projects_per_batch", 5))
    max_chars = int(get_rollup_config(target_dept, "rollup_max_batch_chars", 12000))

    batches = split_rollup_batches(
        rollup_timeline_state,
        max_projects_per_batch=max_projects,
        max_batch_chars=max_chars
    )

    return [
        fallback_rollup_analyze_batch(batch)
        for batch in batches
    ]



def normalize_model_spacing(text):
    """
    修复模型输出中常见的异常空格：
    - 中文字符之间被插入空格
    - 中文和标点之间被插入空格
    ...
    for _ in range(3):
        text = re.sub(r"\b([A-Z0-9]{1,})\s+([A-Z0-9]{2,})\b", r"\1\2", text)
...
def normalize_rollup_markdown(markdown_text):
    """
    修复写入 Workspace 前的 Markdown 格式：
    - 去掉标题 emoji，避免 Workspace 将 emoji 和标题拆行
    - 修复被模型拆开的标题
    - 清理异常空格/零宽字符
    """
    text = normalize_model_spacing(markdown_text)
    ...
    text = re.sub(r"###\s*\n+\s*本周关键进展", "### 本周关键进展", text)
    text = re.sub(r"###\s*\n+\s*本周主要进展", "### 本周主要进展", text)
    text = re.sub(r"###\s*\n+\s*困难及所需帮助", "### 困难及所需帮助", text)
    text = re.sub(r"###\s*\n+\s*下一步计划", "### 下一步计划", text)

    text = re.sub(r"###\s*本周关键进展", "### 本周关键进展", text)
    text = re.sub(r"###\s*本周主要进展", "### 本周主要进展", text)
    text = re.sub(r"###\s*困难及所需帮助", "### 困难及所需帮助", text)
    text = re.sub(r"###\s*下一步计划", "### 下一步计划", text)
...
def run_rollup_for_target_dept(target_dept, dept_by_id, children_map, week_info):
    ...
    try:
        ...
        # 如果模型趋势分析没有抽出任何内容，使用更宽松的 fallback 基于 progress section 重建，避免输出全“暂无”。
        if not has_rollup_content(platform_map):
            print(f"⚠️ [{dept_name}] 趋势分析结果为空，使用 progress fallback 重建 platform_map")
            batch_results = rebuild_rollup_results_with_fallback(rollup_timeline_state, target_dept)
            platform_map = merge_rollup_trend_results(batch_results)
